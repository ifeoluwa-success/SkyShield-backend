...
const SettingsPage: React.FC = () => {
  ...
  const fetchDevices = useCallback(async () => {
    ...
    try {
      ...
      // silently ignore — section hides itself on empty
    } finally {
  ...
  const handleRemoveDevice = async (id: string) => {
    ...
    try {
      ...
      setToast({ type: 'error', message: 'Failed to remove device' });
    }
  };
  ...
  const handleTerminateAll = async () => {
    ...
    try {
      ...
      await fetchSessions();
      setToast({ type: 'success', message: 'All other sessions terminated' });
    } catch {
      setToast({ type: 'error', message: 'Failed to terminate sessions' });
    }
  };
      ...
      <div className="settings-grid">
                  ...
                  <div key={device.id} className="device-row">
                    ...
                    <div className="device-info">
                      ...
                      <span className="device-meta">
                        {[device.browser, device.os, device.ip_address].filter(Boolean).join(' · ')}
                      </span>
                      {device.last_used && (
                        <span className="device-meta">Last used {fmtDate(device.last_used)}</span>
                      )}
                    </div>
                    <div className="device-actions">
                      {device.is_trusted && (
                        <span className="trusted-badge"><CheckCircle size={12} /> Trusted</span>
                      )}
                      <button
                        className={`btn-xs ${device.is_trusted ? 'btn-danger' : 'btn-secondary'}`}
        ...
        <div className="settings-card full-width">
          <div className="card-header between">
            ...
            {sessions.length > 1 && (
              <button className="btn-secondary btn-sm" onClick={handleTerminateAll}>
                Sign out all others
              </button>
            )}
          </div>
                  ...
                  <div key={session.id} className="device-row">
                    ...
                    <div className="device-info">
                      <span className="device-name">
                        {[session.browser, session.os].filter(Boolean).join(' on ') || session.device_info || 'Unknown device'}
                      </span>
                      <span className="device-meta">
                        {session.ip_address && `IP: ${session.ip_address} · `}
                        Signed in {fmtDate(session.login_time)}
                      </span>
                    </div>
                    <div className="device-actions">
                      <button
